        ...
        if isinstance(self.cipher, Affine):
            for i in range(0, self.limit):
                for j in range(0, self.limit):
                    ...
                    #print("Hacking: trying code", i, j, " Decoded word(s): ", decoded_words)
                    for word in decoded_words:
                        if word in self.words and len(word) > 0:
                            match += 1
                    if match > best_match[1]:
                        best_match = (decode, match, i, j)
                    #if best_match[1] == len(decoded_words):  # hvis oversettelsen er perfekt (skaper feil?)
                     #   return best_match
            return best_match

        elif isinstance(self.cipher, Unbreakable):   # Denne kan bruke laaaaang tid
            for i in range(0, int(len(self.words) / 100)):  # yallah
                i = self.words[i]
                print(i)
